#ifndef QUICKFRAMELESSWINDOW_H
#define QUICKFRAMELESSWINDOW_H

#include <dwmapi.h>
#include <QEvent>
#include <QList>
#include <QObject>
#include <QQmlEngine>
#include <QQmlProperty>
#include <QQuickItem>
#include <QQuickWindow>
#include <QTimer>

#include <QColor>

#include <Windows.h>

/* typedef ---------------------------------------------------------------- 80 // ! ----------------------------- 120 */

// Un-documented API – load at runtime
typedef BOOL(WINAPI *PFN_SetWindowCompositionAttribute)(HWND hWnd, struct WINDOWCOMPOSITIONATTRIBDATA *data);

/* enum ------------------------------------------------------------------- 80 // ! ----------------------------- 120 */

// Which attribute we’re setting (only need the one)
enum WINDOW_COMPOSITION_ATTRIB : DWORD { WCA_ACCENT_POLICY = 19 };

// The blur mode
enum ACCENT_STATE : DWORD {
    ACCENT_DISABLED = 0,
    ACCENT_ENABLE_GRADIENT = 1,
    ACCENT_ENABLE_TRANSPARENTGRADIENT = 2,
    ACCENT_ENABLE_BLURBEHIND = 3,        // Aero
    ACCENT_ENABLE_ACRYLICBLURBEHIND = 4, // Acrylic
    ACCENT_INVALID_STATE = 5
};

/* struct ----------------------------------------------------------------- 80 // ! ----------------------------- 120 */

// Data struct we hand to the API
struct ACCENT_POLICY
{
    ACCENT_STATE AccentState;
    DWORD AccentFlags;
    DWORD GradientColor; // 0xAARRGGBB
    DWORD AnimationId;
};

struct WINDOWCOMPOSITIONATTRIBDATA
{
    WINDOW_COMPOSITION_ATTRIB Attribute;
    PVOID Data;
    ULONG SizeOfData;
};

/* class ------------------------------------------------------------------ 80 // ! ----------------------------- 120 */
class WindowEffect
{
public:
    WindowEffect();

    /// Enables acrylic blur behind the window.
    /// \param hWnd       native Win32 HWND (e.g. from QWidget::winId())
    /// \param color      Qt color (AARRGGBB) – will be used directly
    /// \param enableShadow if true, turns on default shadow flags
    /// \param animId     custom animation ID (usually 0)
    void setAcrylicEffect(HWND hWnd, const QColor &color = QColor(0, 0, 0, 0x00), bool enableShadow = true, DWORD animId = 0);

    /// Enables the classic Win10 “blur behind” (Aero)
    void setAeroEffect(HWND hWnd);

    /// Disables acrylic and blur effects
    void disableEffect(HWND hWnd);

    /// Call from mouse-down to allow dragging a frameless window
    void moveWindow(HWND hWnd);

private:
    PFN_SetWindowCompositionAttribute m_pSetWindowCompAttr;
};

const int RESIZEWINDOWBORDERWIDTH = 8;

class QuickFramelessWindow : public QQuickWindow
{
    Q_OBJECT
    QML_ELEMENT
    Q_DISABLE_COPY(QuickFramelessWindow);
    void initStyle();

public:
    explicit QuickFramelessWindow(QObject *parent = nullptr);
    ~QuickFramelessWindow() override;
    Q_INVOKABLE void setWindowTitleBar(QQuickItem *item);
    Q_INVOKABLE void moveCenter(bool isVisible);
    Q_INVOKABLE void showWindowState(const QString &state);
    Q_INVOKABLE Qt::WindowState currentWindowState();
    Q_INVOKABLE void setMainLayout(QQuickItem *item);
    Q_INVOKABLE void setAvctiveWindow();
    Q_INVOKABLE void addWhiteItem(QQuickItem *item);
    Q_INVOKABLE void addWhiteItems(QList<QQuickItem *> items);
    Q_INVOKABLE void disableDoubleClickedMaxWindow(bool is_disable);
    Q_INVOKABLE void setBackgroundColor(const QColor &color);
    Q_INVOKABLE void enableShadow();
    Q_INVOKABLE void disableAcrylicEffect(); // 添加这一行用于手动关闭亚克力效果
signals:

    void windowClose();

protected:
    bool nativeEvent(const QByteArray &eventType, void *message, qintptr *result) Q_DECL_OVERRIDE;
    bool event(QEvent *event) Q_DECL_OVERRIDE;
    qintptr adjustResizeWindow(const HWND &hwnd, const QPoint &point);
    qintptr TackingWindowTitleBar(const QPoint &point);
    bool m_disable = false;
    QQuickItem *titleBar = nullptr;
    qreal dpr;
    QSize screenSize = QSize(0, 0);
    QQuickItem *mainLayoutItem = nullptr;
    QList<QQuickItem *> whitelistItems;

    WindowEffect effect;
};

#endif // QUICKFRAMELESSWINDOW_H
